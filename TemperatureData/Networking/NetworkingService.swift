...
//            } catch {
//                DispatchQueue.main.async {
//                    completion?(error, nil, Optional.none)
//                }
//                return
//            }
//        }
...
//    }
    func connect(type: NetworkingServiceRestType,
                 url: String,
                 inPathParameters pathParameters: [String:String]? = nil,
                 inBodyParametersString bodyParameterString: String?,
        ...
        if let bodyParameterString = bodyParameterString {
            jsonData = bodyParameterString.data(using: .utf8)
        }
    ...
    func connect(type: NetworkingServiceRestType,
        ...
        guard let url = URL(string: add(toPath: url, parameters: pathParameters).addingPercentEncoding(withAllowedCharacters: CharacterSet.urlFragmentAllowed)!) else {
            DispatchQueue.main.async {
                completion?(self.urlError, nil, Optional.none)
            }
            return
        }
    ...
    func connect(url: String,
        ...
        guard let imageData = image.pngData() else {
            DispatchQueue.main.async {
                completion?(self.generateError(code: 0, description: "Image is empty"), nil, Optional.none)
            }
            return
        }
    ...
    func connect(url: String,
        ...
        do {
            ...
            DispatchQueue.main.async {
                completion?(self.generateError(code: 0, description: "Create json file fail"), nil, Optional.none)
            }
            return
        }
...
extension NetworkingService {
    ...
    private func process(data:Data?,
        ...
        if let error = error {
            DispatchQueue.main.async {
                completion?(error, data, Optional.none)
            }
            return
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            DispatchQueue.main.async {
                completion?(self.generateError(code: 0, description: "HTTP request error"), data, Optional.none)
            }
            return
        }
